...
def processData(train, test):
    ...
    Required = ['v4', 'v6', 'v9', 'v10', 'v12', 'v14', 'v16', 
    ...
    features = train.columns[2:]
    todrop = list(set(features).difference(set(Required)))
...
if __name__ == "__main__":
    ...
    gptrain, gptest = processData(train, test)
    ...
    for X_train_index, X_test_index in \
        ...
        TrainingData = gptrain.iloc[X_train_index].copy()
        blindtrain = gptrain.iloc[X_test_index].copy()
        dynamicData = xgb.DMatrix(csr_matrix(TrainingData[features]),
                                    TrainingData.TARGET.values,
                                    silent=True)
        dvisiblevalid = xgb.DMatrix(csr_matrix(blindtrain[features]),
                                    blindtrain.TARGET.values,
                                    silent=True)
        watchlist = [(dynamicData, 'train'), (dvisiblevalid, 'eval')]
        clf = xgb.train(params, dynamicData, num_rounds,
